...
if (fileInput) {
  fileInput.addEventListener("change", function () {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!["pdf", "doc", "docx", "txt", "md", "markdown", "epub"].includes(fileExtension)) {
        alert("Chỉ hỗ trợ tệp .pdf, .doc, .docx, .txt, .md, .markdown, .epub.");
        fileInput.value = "";
        return;
      }
      ...
      } else if (fileExtension === "txt" || fileExtension === "md" || fileExtension === "markdown") {
        icon.className = "fas fa-file-alt";
        icon.style.color = "#888";
      } else if (fileExtension === "epub") {
        icon.className = "fas fa-file-alt";
        icon.style.color = "#29822c";
      } else {
